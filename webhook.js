...
\}\
\
// --- Stock Name API Helpers (getStockNameFromSina, getStockNameFromTencent, etc.) ---\
...
async function getChineseStockName(stockCode) \{\
  let marketPrefix;\
  if (stockCode.length <= 5 && /^\\d+$/.test(stockCode)) \{ // Hong Kong stocks are purely numeric and up to 5 digits\
    marketPrefix = 'hk';\
  \} else if (stockCode.length === 6 && /^\\d+$/.test(stockCode)) \{ // A-shares are 6 digits\
...
\
    ...
    const stockMatch = messageBody.match(/\uc0\u26631 \u30340 :.*\\(([^)]+)\\)/);\
\
    if (stockMatch && stockMatch[1]) \{\
        ...
        if (chineseName) \{\
            finalContent = messageBody.replace(\
                /\uc0\u26631 \u30340 :.*?\\n/, \
                `\uc0\u26631 \u30340 : $\{chineseName\} ($\{stockCode\})\\n`\
            );\
        \}\
    \}\
\
...
\
    if (!forwardResponse.ok) \{\
        ...
        // We still return a success to TradingView, as the proxy itself worked.\
    \}\
\
